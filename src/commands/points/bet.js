...
module.exports = {
  ...
  async execute(msg, args) {
    if (!args[0] || isNaN(args[0])) return msg.reply({ embeds: [sendEmbed(`Please type a number to bet!`)] })

    const player = new Player(msg.member)
    const points = await player.points
    const point = Bot.client.emojis.cache.find(e => e.name === 'pointdiscord')
    const number = Number(args[0])
    if (number > points) return msg.reply({ embeds: [sendEmbed(`You do not have that many points!`)] })
    if (number < 500) return msg.reply({ embeds: [sendEmbed(`You can only bet with 500 points or higher!`)] })
    ...
    if (dicePlayer > diceBot) {
      ...
      await DB.query(`update members set points = ${points - number} where member_id = ${msg.member.id}`)
    } else msg.reply({ embeds: [sendEmbed([
        [msg.author.username, `\`${dicePlayer}\``],
        [msg.guild.me.user.username, `\`${diceBot}\``]
